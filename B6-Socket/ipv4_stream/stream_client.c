...
int main(int argc,const char *argv[])
{
    ...
    if(inet_pton(AF_INET,argv[1],&serv_addr.sin_addr)==-1)//Chuyển đổi địa chỉ IP dạng ký tự sang nhị phân
        handle_error("inet_pton()");
    /*Hàm này chuyển đổi chuỗi ký tự src thành mạng

    cấu trúc địa chỉ trong họ địa chỉ af, sau đó sao chép

    cấu trúc địa chỉ mạng thành dst. Đối số af phải là
